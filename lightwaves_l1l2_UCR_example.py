...
for seed in range(1):
    ...
    dataset = "Worms"
    train_ds = pd.read_csv("Worms_TRAIN.txt", header=None, delimiter="\s+")
    test_ds = pd.read_csv("Worms_TEST.txt", header=None, delimiter="\s+")
    ...
    if orig_rank < total_num_channels:
        ...
        data = {}
        data['train_x'] = np.expand_dims(train_ds.iloc[:, 1:].values.astype("float32"), 1)
        data['train_y'] = train_ds.iloc[:, 0].values.astype("int32")
        data['test_x'] = np.expand_dims(test_ds.iloc[:, 1:].values.astype("float32"), 1)
        data['test_y'] = test_ds.iloc[:, 0].values.astype("int32")
